...
export class UpdateHotelsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.updateHotel = new FormGroup({
      ...
      hotelPrice: new FormControl(this.hPrice, [Validators.pattern('^[1-9][0-9]*$'), Validators.minLength(3), Validators.maxLength(5)]),
  ...
  onSubmit() {
    ...
    this.httpService.put('manager/updatehotel', this.hName, payload)
      .subscribe(
      (response) => {
